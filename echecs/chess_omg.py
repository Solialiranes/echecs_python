...
def are_kings_adjacent(board, start_pos, end_pos, turn):
    # Trouver la position de l'autre roi
    opponent = 'b' if turn == 'w' else 'w'
    ...
    if piece_type == 'P':  # Pion
        ...
        if abs(start_row - end_row) <= 1 and abs(start_col - end_col) <= 1:  # Déplacement d'une case dans n'importe quelle direction
            ...
            if not is_in_check_after_move(board, start_pos, (end_row, end_col), turn):
                # Vérifier que le roi n'est pas adjacent à l'autre roi
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if selected_piece:
                    # Vérifier si le roi est en échec et interdire tout mouvement qui ne le sort pas de l'échec
